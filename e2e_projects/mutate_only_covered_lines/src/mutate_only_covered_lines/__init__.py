...
def mutate_only_covered_lines_multiline(simple_branch: bool) -> str:
    x = (
        "Foo" "Bar"  # coverage.py incorrectly reports this line as not covered. If that changes, tests will break
    )
    ...
    if simple_branch:
        ...
        return f"Hello from mutate_only_covered_lines!" f" (true) {x} {y}"
    else:
        y = [i * 2 for i in range(10) if i % 2 == 0]
        return f"Hello from mutate_only_covered_lines!" f" (false) {x} {y}"
