...
"""
    ...
    def test_no_mutate_class(self):
        source = "class Foo:  # pragma: no mutate block\n    def method(self):\n        return 1 + 1\n"
        no_mutate, ignore_node_lines = parse_pragmas("test.py", source)
        assert no_mutate == set()
        assert ignore_node_lines == {1, 2, 3}

    def test_no_mutate_function(self):
        source = "def foo():  # pragma: no mutate block\n    return 1 + 1\n"
        no_mutate, ignore_node_lines = parse_pragmas("test.py", source)
        assert no_mutate == set()
        assert ignore_node_lines == {1, 2}
    ...
    def test_mixed_pragmas(self):
        ...
        no_mutate, ignore_node_lines = parse_pragmas("test.py", source)
        assert no_mutate == {9}
        assert ignore_node_lines == {1, 2, 3, 5, 6}
    ...
    def test_single_line_function_body(self):
        """def foo(): pass  # pragma: no mutate -- body is a SimpleStatementSuite."""
        source = "def foo(): pass  # pragma: no mutate\n"
        no_mutate, ignore_node_lines = parse_pragmas("test.py", source)
        assert no_mutate == {1}
        assert ignore_node_lines == set()

    def test_single_line_if_body(self):
        source = "if True: pass  # pragma: no mutate\n"
        no_mutate, ignore_node_lines = parse_pragmas("test.py", source)
        assert no_mutate == {1}
        assert ignore_node_lines == set()
...
class TestSelectionPragma:
    ...
    def test_start_end_includes_blank_lines(self):
        source = "# pragma: no mutate start\nx = 1\n\ny = 2\n# pragma: no mutate end\nz = 3\n"
        no_mutate, _ = parse_pragmas("test.py", source)
        assert no_mutate == {1, 2, 3, 4, 5}
...
"""
    ...
    def test_end_without_start_includes_filename(self):
        source = "x = 1\n# pragma: no mutate end\n"
        with pytest.raises(PragmaParseError, match="my_module.py:2"):
