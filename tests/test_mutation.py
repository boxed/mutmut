...
def mutants_for_source(source: str, covered_lines: set[int] | None = None) -> list[str]:
    ...
    mutants: list[str] = [
        cast("cst.Module", module.deep_replace(m.original_node, m.mutated_node)).code for m in mutated_nodes
    ]
    return mutants
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            r'a: str = "Fo\t"',
            [r'a: str = "XXFo\tXX"', r'a: str = "FO\t"', r'a: str = "fo\t"', "a: str = None"],
        ),
        (
...
def test_mangle_function_name():
    ...
    assert (
        mangle_function_name(name="bar", class_name="Foo")
        == f"x{CLASS_NAME_SEPARATOR}Foo{CLASS_NAME_SEPARATOR}bar"
    )
