...
def load_line_spans_index(mutated_file: Path) -> dict[str, LineSpan] | None:
    """Read the ``.spans`` index written next to ``mutated_file`` when it was generated.

    Returns None when there is no index we understand (mutants generated by an older mutmut)."""
    index_path = Path(str(mutated_file) + ".spans")
    try:
        with open(index_path) as f:
            data = json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        return None
    if data.get("version") != MutantLineSpans.format_version:
        return None
    return {name: LineSpan(start, end) for name, (start, end) in data["spans"].items()}


def mutated_method_locations(mutated_file: Path) -> list[MutatedMethodLocation]:
    """Where the generated functions are in ``mutated_file``, sorted by start line.

    Uses the line span index written during generation, so a multi-megabyte mutated file is
    not parsed just to attribute type errors. Falls back to parsing when there is no index."""
    spans = load_line_spans_index(mutated_file)
    if spans is not None:
        locations = [
            MutatedMethodLocation(
                file=mutated_file, function_name=name, line_number_start=span.start, line_number_end=span.end
            )
            for name, span in spans.items()
        ]
    else:
        with open(mutated_file, encoding="utf-8") as file:
            source = file.read()
        wrapper = cst.MetadataWrapper(cst.parse_module(source))
        visitor = MutatedMethodsCollector(mutated_file)
        wrapper.visit(visitor)
        locations = visitor.found_mutants
    # Generated functions never overlap, so once they are ordered by start line the one that
    # contains a given line can be found by binary search (find_method_at_line), which is done
    # once per type error the checker reports for the file.
    locations.sort(key=lambda location: location.line_number_start)
    return locations


def find_method_at_line(
    sorted_locations: list[MutatedMethodLocation], sorted_starts: list[int], line_number: int
) -> MutatedMethodLocation | None:
    """The generated function containing ``line_number``, by binary search.

    ``sorted_locations`` must be ordered by start line (as ``mutated_method_locations`` returns
    them) and ``sorted_starts`` are their start lines, so that bisecting one indexes the other."""
    index = bisect.bisect_right(sorted_starts, line_number) - 1
    if index < 0:
        return None
    location = sorted_locations[index]
    return location if line_number <= location.line_number_end else None
...
def filter_mutants_with_type_checker(checker: TypeCheckerProcess | None = None) -> dict[str, FailedTypeCheckMutant]:
    """Map type errors to the mutants that caused them.

    ``checker`` is a type checker started earlier in the background (with ``mutants/`` as its
    working directory); its report is read here, inside ``mutants/``, so that relative paths
    resolve the same way as when the checker is run right here."""
    with change_cwd(Path("mutants")):
        errors = checker.result() if checker is not None else run_type_checker(config().type_check_command)
        ...
        for path, errors_of_file in errors_by_path.items():
            mutated_methods = mutated_method_locations(path)

            if not mutated_methods:
                continue

            sorted_starts = [m.line_number_start for m in mutated_methods]
            for error in errors_of_file:
                assert error.file_path == path
                mutant = find_method_at_line(mutated_methods, sorted_starts, error.line_number)
                if mutant is None:
                    raise Exception(
