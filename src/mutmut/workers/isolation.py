...
class ActiveWorker(NamedTuple):
    """Info about an active worker, used for timeout checking."""
...
class MutantRunner(ABC):
    ...
    def get_orchestrator_restart_count(self) -> int:
        """Number of orchestrator restarts. Only meaningful for HotForkRunner."""
        return 0
...
class HotForkRunner(MutantRunner):
    ...
    def _restart_orchestrator_with_pending_work(self, exit_code: int = -1) -> None:
        ...
        for mutant_name, (tests, cpu_time_limit, estimated_time, _) in pending_work_copy.items():
            os.write(self.work_pipe_write, pickle.dumps((mutant_name, list(tests), cpu_time_limit)))
    ...
    def get_active_workers(self) -> list[ActiveWorker]:
        """Best-effort view of in-flight mutants (grandchild pids are not visible).

        The orchestrator pid stands in for each pending mutant's worker.
        """
