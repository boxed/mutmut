...
### Added
- add structured run events for mutation sessions
- add `--format json` output for `nootnoot run`

### Changed
- route mutation run diagnostics to stderr to keep stdout clean
...
### Changed
- add explicit mutation state plumbing to avoid global state coupling across runs
- remove unused hash metadata from mutant files to avoid misleading output
