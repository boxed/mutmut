...
def get_unified_diff(argument, dict_synonyms):
    ...
    return _get_unified_diff(source, filename, mutation_id, dict_synonyms)
            ...
            with open(join(report_filename + '.html'), 'w') as f:
                ...
                def print_diffs(status):
                    ...
                    for mutant in sorted(mutants, key=lambda m: m.id):
                        diff = _get_unified_diff(source, filename, MutationID(mutant.line.line, mutant.index, mutant.line.line_number), dict_synonyms)
